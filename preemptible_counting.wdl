...
task preemptible_couting {
    input {
        String ckpt_file_name
    }

    command <<<

        mkdir -p ./local_ckpt_dir
        echo "trial" > ./local_ckpt_dir/tmp.txt
        echo $(ls)
        gsutil rsync ./local_ckpt_dir gs://ld-tmp-storage/Cromwell_CKPT
...
####
####
####        # At the beginning of execution try to localize the file from bucket
####        ckpt =
####        n_start = 0 if ckpt is None else ckpt
####
####
####        for n in range(n_start+1:1000):
####            print(n)
####            ckpt = {"n" : n}
####            with open(local_ckpt.json) as fp:
